...
const Planet = forwardRef(
  (
    ...
    useEffect(() => {
      if (
        glowRef.current &&
        glowRef.current.material instanceof THREE.ShaderMaterial
      ) {
    ...
    // 마커 위치를 저장하기 위한 참조
    const markerPositionsRef = useRef<THREE.Vector3[]>(
      Array(sections.length).fill(new THREE.Vector3())
    );
    ...
    useFrame(({ clock }) => {
      ...
      markerPositionsRef.current.forEach((position, index) => {
        ...
        if (frustumRef.current.intersectsSphere(sphere)) {
          ...
          if (
            distanceToCenter > planetRadius ||
            distanceToMarker < distanceToCenter
          ) {
            // 행성 구체와의 교차 확인
            ...
            // 레이가 행성과 교차하는지 확인
            const doesIntersect = ray.intersectSphere(
              planetSphere,
              intersectionPoint
            );
      ...
      // 시야에 있는 마커 업데이트
      if (
        JSON.stringify(newVisibleMarkers) !== JSON.stringify(visibleMarkers)
      ) {
      ...
      if (!activeSection && (cameraHasMoved || isMobileState)) {
        ...
        // 가장 가까운 마커 인덱스 추출
        const newClosestMarkers = distances
          .slice(0, numMarkersToShow)
          .map((item) => item.index);

        // 이전 상태와 다를 경우에만 업데이트
        if (
          newClosestMarkers.length !== closestMarkers.length ||
          newClosestMarkers.some(
            (marker, idx) => marker !== closestMarkers[idx]
          )
        ) {
          ...
          sections.map((section, index) => (
            <SectionMarker
              ...
              updatePosition={(position) =>
                updateMarkerPosition(index, position)
              }
            />
          ))}
