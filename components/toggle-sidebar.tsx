...
export default function ToggleSidebar({
  ...
  useEffect(() => {
    ...
    // 커스텀 이벤트 발생
    const event = new CustomEvent("mobileSidebarChange", { detail: { isMobile } })
          ...
          {mobileMenuOpen && (
            ...
            >
              ...
              <div className="flex-1 flex flex-col items-center justify-center px-6 pt-16 pb-8">
                {/* 언어 토글 제거 - 헤더에만 표시 */}
                <div className="text-lg font-bold tracking-wider mb-6" style={{ color: contentsColor }}>
                  CONTENTS
                </div>
                ...
                <div className="w-full max-w-sm space-y-4">
                  {sections.map((section, index) => (
                    <div key={section.id} className="w-full">
                      ...
                      >
                        <span className="mr-4 font-sans font-thin tracking-wider">{getSectionNumber(index)}</span>
                        <span className="flex-1">{t(section.title.kr, section.title.en)}</span>
                        <ChevronRight
                          size={16}
                      ...
                      {index < sections.length - 1 && <div className="h-px bg-white/20 w-full mt-2"></div>}
                    </div>
                  ))}
                </div>
            ...
            <div className="text-white h-full relative">
              ...
              <AnimatePresence>
                {isOpen && (
                  ...
                  >
                    <div className="text-sm font-bold tracking-wider mb-2 pl-2" style={{ color: contentsColor }}>
                      CONTENTS
                    </div>

                    <div className="w-full h-px mb-4" style={{ backgroundColor: contentsColor }}></div>
                    ...
                    <div className="flex flex-col">
                      {sections.map((section, index) => (
                        <div key={section.id}>
                          ...
                          >
                            <span className="mr-3 font-sans font-thin tracking-wider">{getSectionNumber(index)}</span>
                            <span className="flex-1">{t(section.title.kr, section.title.en)}</span>
                            <ChevronRight
                              size={14}
                              className={`transition-transform ${
                          ...
                          {index < sections.length - 1 && <div className="h-px bg-white/20 w-full"></div>}
                        </div>
                      ))}
                    </div>
                    ...
                    {sections.map((section, index) => (
                      <motion.button
                        ...
                        className={`p-3 my-2 rounded-full transition-colors duration-200 ${
                          activeSection === section.id ? "bg-white/15" : "bg-transparent hover:bg-white/10"
                        }`}
                        style={{
                          color: activeSection === section.id ? contentsColor : "white",
                        }}
                        onClick={() => onSectionClick(section.id)}
                        whileHover={{ scale: 1.1 }}
                        whileTap={{ scale: 0.95 }}
                      >
                        <span className="font-sans font-thin tracking-wider">{getSectionNumber(index)}</span>
                      </motion.button>
                    ))}
                  </motion.div>
                )}
